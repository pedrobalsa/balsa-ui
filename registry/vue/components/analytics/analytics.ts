...
export type BalsaAnalytics = Plugin & {
  start: () => void;
  stop: () => void;
...
const balsaAnalyticsKey: InjectionKey<BalsaAnalytics> = Symbol("balsa-analytics");
...
export function createBalsaAnalytics(
  ...
  const analytics: BalsaAnalytics = {
    install(app: App): void {
      app.provide(balsaAnalyticsKey, analytics);
      start();
      app.onUnmount(stop);
    },
    start,
...
export function provideBalsaAnalytics(analytics: BalsaAnalytics): void {
  provide(balsaAnalyticsKey, analytics);
}

export function useBalsaAnalytics(): BalsaAnalytics {
  const analytics = inject(balsaAnalyticsKey, undefined);
  if (!analytics) {
