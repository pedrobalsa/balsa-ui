...
When `type="minimal"` is paired with `behavior="reveal"`, Navbar keeps its materialless header. It adds a semantic-background fade behind and below the header only after it returns on upward scroll away from the page origin, keeping navigation readable over page content without affecting the initial view.
...
For a route with a wider page container, pass `floatingLayout="container"` and that container’s CSS maximum width through `floatingMaxWidth`. The slab then follows the route width exactly, while its contents use the same compact 16px, 24px, and 32px insets as the shared site container. The default `inset` layout remains appropriate for a visually detached slab.
...
Navbar remains router-agnostic and renders real anchors. Its `navigate` event carries `(item: NavigationLink, event: MouseEvent)`, so a client router can inspect the original click, call `event.preventDefault()` for an unmodified same-origin navigation, and pass `item.link` to its own navigation API. Modified clicks, external targets, and native fallback behavior remain available because Navbar does not cancel the event itself.
