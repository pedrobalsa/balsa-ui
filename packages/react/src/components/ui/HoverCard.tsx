...
export type HoverCardVariant = LayerVariant;

export interface HoverCardProps extends Omit<HTMLAttributes<HTMLElement>, "id"> {
  id: string;
  label: string;
  "data-balsa"?: string;
  "data-palette"?: string;
...
const variantClasses: Readonly<Record<HoverCardVariant, string[]>> = {
...
export function HoverCard(rawProps: HoverCardProps) {
  const { props, theme } = useResolvedThemeProps("hover-card", "overlays", rawProps, {
    variant: "surface",
    rounded: "lg",
    shadow: "auto",
  } as const);
